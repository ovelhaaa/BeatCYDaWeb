...
class BassGroove {
  ...
  computeTriggerProbability(context) {
    ...
    if (context.isDownbeat) {
      ...
      probability = Math.max(probability, 0.8 + this.params.density * 0.2);
    } else {
      probability *= 0.3;
    }
...
class AudioEngine {
  ...
  buildKickBuffer(event) {
    ...
    const velocity = typeof event === "number" ? event : event.velocity;
    const accent = typeof event === "object" ? event.accent : false;
    const ghost = typeof event === "object" ? event.ghost : false;
    ...
    const pitchMod = accent ? 1.1 : ghost ? 0.9 : 1.0;
    const clickMod = accent ? 1.5 : ghost ? 0.3 : 1.0;

    const baseFreq = (35 + params.pitch * 50) * pitchMod;
    const gainMul = 1 - (0.00025 + (1 - params.decay) * 0.002);
    const pitchMul = 0.96;
    const sweepAmount = (150 + params.timbre * 350) * pitchMod;
    const driveFactor = 1 + params.drive * 4;
    const cutoffBase = 0.04 + params.drive * 0.08;
    const svfQ = 0.5 + params.timbre * 0.4;

    let envGain = velocity * (ghost ? 0.7 : 1.0);
    let envPitch = 1.0;
    let clickEnv = 1.0 * clickMod;
    let phase = 0.0;
    let svfLow = 0.0;
    let svfBand = 0.0;
    let dcX1 = 0.0;
  ...
  buildSnareBuffer(event) {
    ...
    const dynTimbre = params.timbre * (0.5 + velocity * 0.5);
    ...
    if (params.mode === 2) {
      const envMulNoise = calculateEnvMul((100 + dynDecay * 300) * 0.001, sr);
      let envNoise = velocity * (ghost ? 0.3 : 1.0);
      let wire1 = 0.0;
      let burstCount = 4;
      let burstSamples = 0;
    ...
    const ghostMod = ghost ? 0.4 : 1.0;
    const bodyMs = 80 + dynDecay * 170;
    const wireMs = 60 + dynDecay * 390;
    const clickMs = 1.5 + dynTimbre * 1.5;
    const pitchMs = 15;
    ...
    let envBody = velocity * ghostMod;
    let envNoise = velocity * (ghost ? 0.2 : 1.0);
    let envClick = velocity * (ghost ? 0.1 : 1.0);
    let envPitch = 1.0;
    let wire1 = 0.0;
    let wire2 = 0.0;
    let dcX1 = 0.0;
  ...
  buildHatBuffer(event, open) {
    ...
    const dynDecay = params.decay * (ghost ? 0.6 : accent ? 1.1 : 1.0);
    const dynTimbre = params.timbre * (ghost ? 0.5 : accent ? 1.2 : 1.0);

    const envMul = calculateEnvMul((minMs + dynDecay * (maxMs - minMs)) * 0.001, sr);
    const baseFreq = 300 + dynTimbre * 300;
    const ratios = [1.0, 1.48, 2.15, 3.71];
    const phase = [0, 0, 0, 0];
    const inc = ratios.map((ratio) => baseFreq * ratio * srInv);
    let env = velocity * (ghost ? 0.5 : 1.0);
    let svfLow = 0.0;
    let svfBand = 0.0;
  ...
  trigger(trackIndex, event, bassEvent = null) {
    if (!this.ctx) return;
    const vel = (typeof event === "number") ? event : event.velocity;
...
class PenosaDesktopSim {
  ...
  restoreSlots() {
    ...
    // Fallback to v1 slots if v2 is not found
    if (!saved) {
  ...
  syncBassFromGlobals() {
    ...
    // Ensure that candidate stays within min-max
    candidate = clamp(candidate, BASS_ROOT_MIN, BASS_ROOT_MAX);
  ...
  syncBassFromPitch(pitch) {
    ...
    const newRoot = this.pitchToBassRootNote(this.bassVoicePitch);
    this.bassGroove.updateParams({ rootNote: newRoot });
    this.voiceParams[VOICE_BASS].pitch = this.bassVoicePitch;
  ...
  tick() {
    ...
    for (let i = 0; i < TRACK_COUNT; i += 1) {
      ...
      if (i !== VOICE_BASS) {
        this.audio.trigger(i, eventObj);
        events.push(`${TRACK_NAMES[i]}:${velocity.toFixed(2)}${accent ? ":accent" : ghost ? ":ghost" : ""}`);
      }
    }
...
function updateUi() {
  ...
  document.getElementById("debugSummary").textContent = [
    ...
    `last tick events:`,
    `${sim.lastTickEvents.length ? sim.lastTickEvents.join("\n") : "  silence"}`,
    "",
    "bass decision",
    `reason: ${debug.reason}`,
